...
migrate((app) => {
  ...
  collection.indexes.push("CREATE UNIQUE INDEX idx_payments_m_payment_id ON payments (m_payment_id)");
  collection.indexes.push("CREATE UNIQUE INDEX idx_payments_sessionId ON payments (sessionId)");
  return app.save(collection);
}, (app) => {
  try {
  const collection = app.findCollectionByNameOrId("payments");
  collection.indexes = collection.indexes.filter(idx => !idx.includes("idx_payments_m_payment_id"));
